...
export function ExpenseSpreadsheet({
  ...
  return (
    <div className="border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 rounded-sm overflow-hidden">
      {/* Header with title */}
      <div className={cn('flex items-center justify-between px-3 py-1.5 border-b border-slate-300 dark:border-slate-600', titleColor)}>
      ...
      {/* Column headers */}
      <div className="grid grid-cols-[1fr_100px_100px_40px] border-b border-slate-300 dark:border-slate-600 bg-slate-100 dark:bg-slate-700">
        <div className="px-2 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-300 border-r border-slate-300 dark:border-slate-600 flex items-center gap-1">
          Keperluan <ChevronDown className="w-3 h-3" />
        </div>
        <div className="px-2 py-1.5 text-xs font-medium text-slate-600 dark:text-slate-300 border-r border-slate-300 dark:border-slate-600 flex items-center gap-1">
      ...
      <div className="divide-y divide-slate-200 dark:divide-slate-600">
        {visibleTransactions.map((transaction) => {
          ...
          return (
            <div 
              key={transaction.id} 
              className={cn(
                'grid grid-cols-[1fr_100px_100px_40px] hover:bg-slate-50 dark:hover:bg-slate-700/50',
            ...
            >
              ...
              <div className="border-r border-slate-200 dark:border-slate-600 min-h-[32px]">
                ...
                >
                  <SelectTrigger className="h-full w-full border-0 rounded-none text-sm px-2 py-1 focus:ring-0 focus:ring-offset-0 shadow-none hover:bg-blue-50 dark:hover:bg-slate-600/50">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {categories.map((cat) => (
        ...
        {newRows.map((row) => (
          <div 
            key={row.tempId} 
            className="grid grid-cols-[1fr_100px_100px_40px] bg-green-50 dark:bg-green-900/20"
          >
            {/* Description cell */}
            <div className="border-r border-slate-200 dark:border-slate-600 min-h-[32px]">
              {editingCell?.id === row.tempId && editingCell?.field === 'description' ? (
                ...
                <div
                  ...
                  className="px-2 py-1 text-sm cursor-cell hover:bg-blue-50 dark:hover:bg-slate-600/50 h-full flex items-center"
                >
                  {row.description || <span className="text-slate-400 italic">Enter description</span>}
                </div>
              )}
            </div>
        ...
        {Array.from({ length: Math.max(0, 1 - visibleTransactions.length - newRows.length) }).map((_, i) => (
          <div 
            key={`empty-${i}`} 
            className="grid grid-cols-[1fr_100px_100px_40px] hover:bg-slate-50 dark:hover:bg-slate-700/50"
          >
      ...
      {/* Total row */}
      <div className="grid grid-cols-[1fr_100px_100px_40px] border-t-2 border-slate-400 dark:border-slate-500 bg-slate-100 dark:bg-slate-700">
